...
namespace MerelyCapes
{
    ...
    internal static class CapeThumbRenderer
    {
        ...
        public static Bitmap GenerateThumbnail(Bitmap? capeBitmap, int width = 256, int height = 256)
        {
            ...
            if (hasCape)
                UploadTexture(gl, texIds[1], capeBitmap!);

            // ── Projection ───────────────────────────────────────────────── //
            // CTE bbmodel camera: fov=40, aspect=1:1
            gl.MatrixMode(OpenGL.GL_PROJECTION);
            gl.LoadIdentity();
            gl.Perspective(40.0, (double)width / height, 0.1, 500.0);

            // ── ModelView ────────────────────────────────────────────────── //
            // Camera from CTE bbmodel camera element:
            //   position = [-17.575, 26.975, 20.45]
            //   rotation = [-17.75, -37, 0]
            //
            // OpenGL view transform = negate rotations, negate position:
            //   Rotate(+17.75, X) -> Rotate(+37, Y) -> Translate(+17.575, -26.975, -20.45)
            //
            // The cape cube in CTE has rotation [12.5, -180, 0] which points its
            // visible north face toward this camera (positive Z side).
            ...
            // ── Two-pass alpha extraction (white bg then black bg) ───────── //
            ...
            // ── Downsample (this is the antialiasing step) ───────────────── //
        ...
        private static byte[] ReadRawPixels(OpenGL gl, int width, int height)
        {
            ...
            try { gl.ReadPixels(0, 0, width, height, OpenGL.GL_RGBA, OpenGL.GL_UNSIGNED_BYTE, pixels); }
            finally { h.Free(); }

            // Flip Y (GL is bottom-up)
            byte[] flipped = new byte[pixels.Length];
            int rowBytes = width * 4;
            for (int y = 0; y < height; y++)
                Buffer.BlockCopy(pixels, (height - 1 - y) * rowBytes, flipped, y * rowBytes, rowBytes);
        ...
        //  UV helpers
        //
        //  Player skin atlas: 64x64  -> both u and v divide by 64
        //  Cape atlas:        64x32  -> u divides by 64, v divides by 32
        // ------------------------------------------------------------------ //
        ...
        private static float Cv(float py) => py / 32f;

        // ------------------------------------------------------------------ //
        //  Body part geometry
        //
        //  All coordinates taken directly from the persona bbmodel (same model
        //  the plugin uses for the base character). UVs taken from that same
        //  model's face data, normalised to [0,1] above.
        // ------------------------------------------------------------------ //

        // ── HEAD ── from[-5,24,-4] to[3,32,4]
        ...
        //  Cape geometry
        //
        //  Taken directly from the CTE bbmodel belt/cube:
        //    from=[-5,8,3]  to=[5,24,4]
        //    origin=[0,24,3]  rotation=[12.5, -180, 0]
        //
        //  The Y=-180 rotation mirrors the cape so that its north face points
        //  toward the camera (positive Z side). The X=12.5 tilt drapes it
        //  naturally off the shoulders.
        //
        //  UV atlas is 64x32. The NORTH face is the visible outer face.
        //  All UVs normalised: Cu = px/64, Cv = py/32.
        // ------------------------------------------------------------------ //
        private static void DrawCape(OpenGL gl)
        {
            gl.PushMatrix();

            // Rotate around origin shifted -1 on X to align with body center (x=-1)
            gl.Translate(-1f, 24f, 3f);
            gl.Rotate(-12.5f, 1f, 0f, 0f);   // X tilt (drape)
            gl.Rotate(-180f, 0f, 1f, 0f);   // Y flip (face toward camera)
            gl.Translate(1f, -24f, -3f);

            // Cape spans -6 to +4 on X (center=-1, matching body center)
            float x1 = -6f, x2 = 4f;
            float y1 = 8f, y2 = 24f;
            float z1 = 3f, z2 = 4f;

            // North (outer face visible to camera)  uv[1,1,11,17]
            // U coords swapped to counter the horizontal mirror from Y=-180 rotation
            Quad(gl, Cu(11), Cv(1), Cu(1), Cv(17),
                x1, y2, z1, x2, y2, z1, x2, y1, z1, x1, y1, z1);

            // South (inner face)  uv[12,1,22,17]
            Quad(gl, Cu(12), Cv(1), Cu(22), Cv(17),
                x2, y2, z2, x1, y2, z2, x1, y1, z2, x2, y1, z2);

            // East  uv[0,1,1,17]
            Quad(gl, Cu(0), Cv(1), Cu(1), Cv(17),
                x2, y2, z1, x2, y2, z2, x2, y1, z2, x2, y1, z1);

            // West  uv[11,1,12,17]
            Quad(gl, Cu(11), Cv(1), Cu(12), Cv(17),
                x1, y2, z2, x1, y2, z1, x1, y1, z1, x1, y1, z2);

            // Up    uv[11,1 -> 1,0]
            QuadFlipped(gl, Cu(11), Cv(1), Cu(1), Cv(0),
                x1, y2, z2, x2, y2, z2, x2, y2, z1, x1, y2, z1);

            // Down  uv[21,0,11,1]
            QuadFlipped(gl, Cu(21), Cv(0), Cu(11), Cv(1),
        ...
        /// <summary>
        /// Emits a quad. Vertices in order: top-left, top-right, bottom-right, bottom-left.
        /// UVs span from (u1,v1) top-left to (u2,v2) bottom-right.
        /// </summary>
        ...
        /// <summary>
        /// Same as Quad but handles bbmodel "flipped" UV regions where u1>u2 or v1>v2
        /// (used on top/bottom faces).
        /// </summary>
        ...
        private static void UploadTexture(OpenGL gl, uint texId, Bitmap bmp)
        {
            ...
            try
            {
                // 0x80E1 = GL_BGRA — matches System.Drawing ARGB in little-endian
        ...
        //  Skin loader
        //
        //  Loads persona.png from the executable directory.
        //  The plugin builds this skin at runtime from the project's body and
        //  head textures composited over the base persona layer — drop whatever
        //  PNG the plugin would produce here under the name "persona.png".
        // ------------------------------------------------------------------ //

        private static Bitmap LoadPersonaSkin()
        {
            string path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "persona.png");
            if (File.Exists(path))
            {
                try
                {
        ...
        private static Bitmap CreateFallbackSkin()
        {
            ...
            g.Clear(Color.Transparent);
            // Head
            g.FillRectangle(new SolidBrush(Color.FromArgb(255, 198, 133, 66)), new Rectangle(8, 8, 8, 8));
            // Body
            g.FillRectangle(new SolidBrush(Color.FromArgb(255, 70, 100, 160)), new Rectangle(20, 20, 8, 12));
            // Right arm
            g.FillRectangle(new SolidBrush(Color.FromArgb(255, 198, 133, 66)), new Rectangle(44, 20, 4, 12));
            // Left arm
            g.FillRectangle(new SolidBrush(Color.FromArgb(255, 198, 133, 66)), new Rectangle(36, 52, 4, 12));
            // Right leg
            g.FillRectangle(new SolidBrush(Color.FromArgb(255, 60, 80, 130)), new Rectangle(4, 20, 4, 12));
            // Left leg
            g.FillRectangle(new SolidBrush(Color.FromArgb(255, 60, 80, 130)), new Rectangle(20, 52, 4, 12));
            return bmp;
        }
    }
